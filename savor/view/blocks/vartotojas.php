...
HTML;
	...
	if ( isset( $conf['puslapiai']['view_user.php']['id'] ) ) {
		...
		$text .= "<li><a href=\"" . url( "?id," . $conf['puslapiai']['view_user.php']['id'] ) . $bruksniukxs . $_SESSION[SECRET]['username'] . "\"><img src=\"images/icons/user-white.png\" alt=\"@\" style=\"vertical-align: middle;\" border=\"0\" /> {$lang['user']['profile']}</a></li>";
	}
	...
	//Pm nuoroda
	if ( puslapis( 'pm.php' ) ) {
		$pm = kiek( 'private_msg', "WHERE `to`=" . escape( $_SESSION[SECRET]['username'] ) . " AND `read`='NO'", 'total' );
	...
	if ( puslapis( 'moderatorius.php' ) && !empty( $_SESSION[SECRET]['mod'] ) ) {
		$text .= "
	...
	";
	$sTitle = sprintf( $lang['user']['hello'], $_SESSION[SECRET]['username'] );
